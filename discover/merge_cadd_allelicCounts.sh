...
[ -d ${srcDir} ] && echo "Find ${srcDir}" || mkdir -p ${srcDir}

dstDir=${WKHM}/annotCadd/
[ -d ${dstDir} ] && echo "Find ${dstDir}" || mkdir -p ${dstDir}
...
for x in {1..22}; do
	...
	srun -n 1 -c 1 --mem=10G -e chr${x}.err -o chr${x}.out \
        join -1 2 -2 3 -t $'\t' \
            <(sed -n "/^${x}\t/p" ${srcDir}/allelicCountsPerChrCov10_noheader.tsv | sort -k2,2n) \
            <(sed -n "/^${x}\t/p" ${srcDir}/allelicCountsPerChrCov10.bed | sort -k2,2n) \
            | awk -F "\t" '{nf=split($NF, arr, "_"); if(arr[1]==$2 && arr[3]==$3 && arr[4]==$4 && arr[5]!=arr[6]){printf $2"\t"$1"\t"; for(i=3; i<NF-2;i++){printf $i"\t"}; for(j=1;j<=6;j++){printf arr[j]"\t"}; printf arr[7]; for(k=8;k<=nf;k++){printf "_"arr[k]}; printf "\n"}}' \
